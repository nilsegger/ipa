...
class PersonalListController(ListController):
    ...
    async def _select_stmt(self, limit, offset, join_foreign_keys) -> str:
        ...
        if join_foreign_keys:
            return """SELECT personal.uuid AS "uuid", name, logins.username AS "benutzername", logins.role AS "rolle" FROM personal JOIN logins on personal.uuid = logins.uuid"""
        else:
            return """SELECT uuid, name, FROM personal JOIN logins on personal.uuid = logins.uuid"""
